	...
	constructor(client, request) {
		...
		if (request.url.host === this.client.restClient.baseUrl.host && request.route) {
			...
			if (this.client.buckets.has(bucket)) {
				bucket = this.client.buckets.get(bucket);
			} else {
		...
		return new Promise((resolve, reject) => {
			if (this.bucket) {
				...
				if (this.bucket.ratelimit.remaining === 1) {
					...
					if (this.request.route.path === RestEndpoints.CHANNELS.MESSAGE_REACTION_USER && diff === 1000) {
						diff = 250;
			...
			return new Promise((resolve, reject) => {
				if (this.bucket) {
					...
					if (this.bucket.ratelimit.remaining === undefined) {
						this.bucket.ratelimit.remaining = remaining;
					} else if (this.bucket.ratelimit.remaining >= remaining) {
						this.bucket.ratelimit.remaining = remaining;
					} else {
						this.bucket.ratelimit.remaining--;
					}
				...
				if (response.statusCode === 502 && this.retries++ >= this.maxRetries) {
					setTimeout(() => {
			...
			if (!response.ok) {
				let message = `HTTP Exception: ${response.statusCode}`;
				let code;
				if (this.bucket && response.data && response.data.code) {
