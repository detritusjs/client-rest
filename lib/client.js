...
class Client {
	...
	ackChannel(channelId, messageId, token) {
		return new Promise((resolve, reject) => {
			...
			this.request({
				route: {method: 'post', path: RestEndpoints.CHANNELS.MESSAGE_ACK, params},
				body
			}).then(resolve).catch(reject);
		});
	}
	...
	ackGuild(guildId) {
		return new Promise((resolve, reject) => {
			...
			this.request({
				route: {method: 'post', path: RestEndpoints.GUILDS.ACK, params}
			}).then(resolve).catch(reject);
		});
	}
	...
	bulkDeleteMessages(channelId, messageIds) {
		return new Promise((resolve, reject) => {
			const params = Tools.checkData({channelId, messageIds}, {
				channelId: {type: 'snowflake', required: true},
	...
	createDm(recipient_id) {
		return new Promise((resolve, reject) => {
			this.request({
				...
				body: Tools.checkData({recipient_id}, {recipient_id: {type: 'snowflake', required: true}})
			}).then(resolve).catch(reject);
		});
	}
	...
	editGuildChannelPositions(guildId, body) {
		return new Promise((resolve, reject) => {
			...
			body = body.map((channel) => Tools.checkData(channel, {id: {type: 'snowflake', required: true}, position: {type: 'integer', required: true}}));
	...
	editGuildRolePositions(guildId, body) {
		return new Promise((resolve, reject) => {
			...
			body = body.map((role) => Tools.checkData(role, {id: {type: 'snowflake', required: true}, position: {type: 'integer', required: true}}));
	...
	fetchActivityMetadata(userId, sessionId) {
		return new Promise((resolve, reject) => {
			...
			this.request({
				route: {method: 'get', path: RestEndpoints.USERS.ACTIVITY_METADATA, params}
			}).then(resolve).catch(reject);
		});
	}
	...
	fetchApplication(applicationId) {
		return new Promise((resolve, reject) => {
			...
			this.request({
				route: {method: 'get', path: RestEndpoints.APPLICATIONS.ID, params}
			}).then(resolve).catch(reject);
		});
	}
	...
	fetchOauth2Application(applicationId) {
		return new Promise((resolve, reject) => {
			...
			const params = Tools.checkData({applicationId}, {channelId: {type: 'snowflake', required: true}});

			this.request({
				route: {method: 'get', path: RestEndpoints.OAUTH2.APPLICATIONS.ID, params}
			}).then(resolve).catch(reject);
		});
	}
	...
	fetchOauth2Token(tokenId) {
		return new Promise((resolve, reject) => {
			...
			this.request({
				route: {method: 'get', path: RestEndpoints.OAUTH2.TOKEN, params}
			}).then(resolve).catch(reject);
		});
	}
	...
	sendDownloadEmail(email) {
		return new Promise((resolve, reject) => {
			this.request({
				...
				body: Tools.checkData({email}, {email: {type: 'string', required: true}})
			}).then(resolve).catch(reject);
		});
	}
	...
	sendDownloadText(number) {
		return new Promise((resolve, reject) => {
			this.request({
				...
				body: Tools.checkData({number}, {number: {type: 'string', required: true}})
			}).then(resolve).catch(reject);
		});
	}
	...
	sendFriendRequest(body) {
		return new Promise((resolve, reject) => {
			...
			this.request({
				route: {method: 'post', path: RestEndpoints.USERS.RELATIONSHIPS},
				body
			}).then(resolve).catch(reject);
		});
	}
	...
	callFetch(channelId) {
		return new Promise((resolve, reject) => {
			...
			this.request({
				route: {method: 'GET', path: RestEndpoints.CHANNELS.CALL, params},
			}).then(resolve).catch(reject);
		});
	}

	callStartRing(channelId, body) {
		return new Promise((resolve, reject) => {
			const params = Tools.checkData({channelId}, {channelId: {type: 'snowflake', required: true}});
			...
			this.request({
				route: {method: 'post', path: RestEndpoints.CHANNELS.CALL_RING, params},
				body
			}).then(resolve).catch(reject);
		});
	}

	callStopRing(channelId, body) {
		return new Promise((resolve, reject) => {
			const params = Tools.checkData({channelId}, {channelId: {type: 'snowflake', required: true}});
			...
			this.request({
				route: {method: 'post', path: RestEndpoints.CHANNELS.CALL_STOP_RINGING, params},
				body
			}).then(resolve).catch(reject);
		});
	}
